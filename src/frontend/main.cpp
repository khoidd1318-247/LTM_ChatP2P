...
// -----------------------------------------------------------------------------
// FRIEND FINDER / LAN ROOM DISCOVERY (ADDED MODULE)
// -----------------------------------------------------------------------------
// This module is intentionally separate from the existing TCP chat protocol.
// It uses UDP port 9090 only for LAN discovery and never changes the existing
// TCP chat port or [HANDSHAKE]/[MSG]/... packets.
static constexpr unsigned short FRIEND_FINDER_UDP_PORT = 9090;
static constexpr int FRIEND_FINDER_TIMEOUT_SECONDS = 6;
static constexpr int FRIEND_FINDER_BROADCAST_INTERVAL_MS = 1500;

struct FriendFinderPeer {
    string user_id;
    string name;
    string role;
    string ip;
...
mutex friend_finder_mutex;
vector<FriendFinderPeer> friend_finder_peers;
string friend_finder_search = "";

asio::io_context* friend_finder_io = nullptr;
udp::socket* friend_finder_socket = nullptr;
thread* friend_finder_thread = nullptr;
asio::steady_timer* friend_finder_cleanup_timer = nullptr;

chrono::steady_clock::time_point friend_finder_last_broadcast;

bool isValidPort(const string& portStr, int& outPort);

void FriendFinder_Start();
void FriendFinder_Stop();
void FriendFinder_Tick();
void FriendFinder_RequestBroadcast();
void FriendFinder_RenderUI();
void render_avatar(const string& name, const string& id, float size);
void start_connecting(string ip, int port);

string FriendFinder_Sanitize(const string& value) {
    string result = value;
...
void FriendFinder_RemoveExpired() {
    lock_guard<mutex> lock(friend_finder_mutex);
    auto now = chrono::steady_clock::now();
    friend_finder_peers.erase(
        remove_if(friend_finder_peers.begin(), friend_finder_peers.end(),
            [&](const FriendFinderPeer& peer) {
                return chrono::duration_cast<chrono::seconds>(
                    now - peer.last_seen)
                    .count() > FRIEND_FINDER_TIMEOUT_SECONDS;
            }),
        friend_finder_peers.end());
}

void FriendFinder_StartReceive() {
    if (!friend_finder_socket || !friend_finder_socket->is_open())
        return;

    auto buffer = make_shared<array<char, 2048>>();
    auto sender = make_shared<udp::endpoint>();

    friend_finder_socket->async_receive_from(
        asio::buffer(*buffer), *sender,
        [buffer, sender](const asio::error_code& ec, size_t bytes) {
            if (!ec && bytes > 0) {
                string packet(buffer->data(), bytes);
                stringstream ss(packet);
                string tag, remote_id, remote_name, remote_role, remote_port;

                getline(ss, tag, '|');
                getline(ss, remote_id, '|');
                getline(ss, remote_name, '|');
                getline(ss, remote_role, '|');
                getline(ss, remote_port, '|');

                int tcp_port = 0;
                bool valid_port = true;
                try {
                    tcp_port = stoi(remote_port);
                }
                catch (...) {
                    valid_port = false;
                }

                if (tag == "P2P_FRIEND_V1" && !remote_id.empty() &&
                    remote_id != local_user_id && valid_port && tcp_port >= 1 &&
                    tcp_port <= 65535) {
                    lock_guard<mutex> lock(friend_finder_mutex);
                    bool found = false;

                    for (auto& peer : friend_finder_peers) {
                        if (peer.user_id == remote_id) {
                            peer.name = remote_name;
                            peer.role = remote_role;
                            peer.ip = sender->address().to_string();
                            peer.tcp_port = tcp_port;
                            peer.last_seen = chrono::steady_clock::now();
                            found = true;
                            break;
                        }
                    }
                    ...
                    if (!found) {
                        FriendFinderPeer peer;
                        peer.user_id = remote_id;
                        peer.name = remote_name.empty() ? "Anonymous" : remote_name;
                        peer.role = remote_role;
                        peer.ip = sender->address().to_string();
                        peer.tcp_port = tcp_port;
                        peer.last_seen = chrono::steady_clock::now();
                        friend_finder_peers.push_back(peer);
                    }
                }
            }

            if (friend_finder_socket && friend_finder_socket->is_open())
                FriendFinder_StartReceive();
        });
}
...
void FriendFinder_Start() {
    ...
    try {
        friend_finder_io = new asio::io_context();
        friend_finder_socket = new udp::socket(*friend_finder_io);

        friend_finder_socket->open(udp::v4());
        friend_finder_socket->set_option(udp::socket::reuse_address(true));
        friend_finder_socket->set_option(udp::socket::broadcast(true));
        friend_finder_socket->bind(udp::endpoint(udp::v4(), FRIEND_FINDER_UDP_PORT));

        FriendFinder_StartReceive();

        friend_finder_thread = new thread([]() {
            try {
                if (friend_finder_io)
                    friend_finder_io->run();
            }
            catch (...) {
            }
            });

        friend_finder_last_broadcast = chrono::steady_clock::now() -
            chrono::milliseconds(
                FRIEND_FINDER_BROADCAST_INTERVAL_MS);
    }
    catch (const exception& e) {
        if (friend_finder_socket) {
            asio::error_code ec;
            friend_finder_socket->close(ec);
            delete friend_finder_socket;
            friend_finder_socket = nullptr;
        }
        if (friend_finder_io) {
            friend_finder_io->stop();
            delete friend_finder_io;
            friend_finder_io = nullptr;
        }
        friend_finder_thread = nullptr;
    }
}
...
void FriendFinder_Stop() {
    ...
    if (friend_finder_thread && friend_finder_thread->joinable()) {
        friend_finder_thread->join();
        delete friend_finder_thread;
        friend_finder_thread = nullptr;
    }
    ...
    lock_guard<mutex> lock(friend_finder_mutex);
    friend_finder_peers.clear();
}

void FriendFinder_RequestBroadcast() {
    if (!friend_finder_io || !friend_finder_socket ||
        !friend_finder_socket->is_open())
        return;

    // Only an active Host advertises a room. Normal Peer/IDLE instances still
    // listen for hosts, so the user can discover and join them directly.
    if (local_role != "Host" || currentState == IDLE)
        return;

    int tcp_port = 0;
    string port_text = portBuf;
    if (!isValidPort(port_text, tcp_port))
        return;

    string display_name = (strlen(username) > 0) ? string(username) : "Anonymous";
    string packet = "P2P_FRIEND_V1|" + local_user_id + "|" +
        FriendFinder_Sanitize(display_name) + "|" + local_role + "|" +
        to_string(tcp_port);

    // All UDP socket operations are posted to the discovery io_context so the
    // receive handler and send operation do not touch the socket concurrently.
    asio::post(*friend_finder_io, [packet]() {
        if (!friend_finder_socket || !friend_finder_socket->is_open())
            return;
        asio::error_code ec;
        udp::endpoint broadcast_endpoint(asio::ip::address_v4::broadcast(),
            FRIEND_FINDER_UDP_PORT);
        friend_finder_socket->send_to(asio::buffer(packet), broadcast_endpoint, 0,
            ec);
        });
}

void FriendFinder_Tick() {
    FriendFinder_RemoveExpired();

    auto now = chrono::steady_clock::now();
    if (chrono::duration_cast<chrono::milliseconds>(
        now - friend_finder_last_broadcast)
        .count() >= FRIEND_FINDER_BROADCAST_INTERVAL_MS) {
        FriendFinder_RequestBroadcast();
        friend_finder_last_broadcast = now;
    }
}
...
void FriendFinder_RenderUI() {
    ...
    ImGui::TextColored(ImVec4(0.45f, 0.75f, 1.0f, 1.0f),
        "Find Friends / Rooms (LAN)");
    ImGui::TextColored(ImVec4(0.55f, 0.60f, 0.68f, 1.00f),
        "Discover active Host rooms automatically on the same local network.");

    ImGui::PushItemWidth(260.0f);
    static char friend_search_buf[128] = "";
    ImGui::InputTextWithHint("##friendsearch", "Search by name or ID...",
        friend_search_buf, IM_ARRAYSIZE(friend_search_buf));
    ImGui::PopItemWidth();
    ImGui::SameLine();
    ...
    string filter = friend_search_buf;
    ...
    for (const auto& peer : friend_finder_peers) {
        string name_lower = peer.name;
        string id_lower = peer.user_id;
        transform(name_lower.begin(), name_lower.end(), name_lower.begin(),
            [](unsigned char c) { return static_cast<char>(tolower(c)); });
        transform(id_lower.begin(), id_lower.end(), id_lower.begin(),
            [](unsigned char c) { return static_cast<char>(tolower(c)); });

        if (!filter.empty() && name_lower.find(filter) == string::npos &&
            id_lower.find(filter) == string::npos)
            continue;

        displayed = true;
        ImGui::PushID(peer.user_id.c_str());
        ImGui::BeginChild("##friendrow", ImVec2(0, 62), true);
        render_avatar(peer.name, peer.user_id, 34.0f);
        ImGui::SameLine();
        ImGui::BeginGroup();
        ImGui::TextColored(ImVec4(0.85f, 0.90f, 1.00f, 1.00f), "%s",
            peer.name.c_str());
        ImGui::TextColored(ImVec4(0.55f, 0.60f, 0.68f, 1.00f),
            "%s  |  %s:%d", peer.user_id.c_str(), peer.ip.c_str(),
            peer.tcp_port);
        ImGui::EndGroup();
        ImGui::SameLine(ImGui::GetWindowWidth() - 120.0f);
        if (ImGui::Button("Join Directly", ImVec2(105, 30))) {
            snprintf(targetIP, sizeof(targetIP), "%s", peer.ip.c_str());
            snprintf(joinPortBuf, sizeof(joinPortBuf), "%d", peer.tcp_port);
            string selected_ip = peer.ip;
            int selected_port = peer.tcp_port;
            ImGui::EndChild();
            ImGui::PopID();
            // Do not hold friend_finder_mutex while starting the TCP connection.
            start_connecting(selected_ip, selected_port);
            return;
        }
        ImGui::EndChild();
        ImGui::PopID();
    ...
    if (!displayed) {
        ImGui::TextColored(ImVec4(0.55f, 0.60f, 0.68f, 1.00f),
            "No active rooms found yet. Make sure your friend is hosting.");
    }
}
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (currentState == IDLE) {
            ...
            // --- ADDED: FRIEND FINDER UI ---
            FriendFinder_RenderUI();
        }
