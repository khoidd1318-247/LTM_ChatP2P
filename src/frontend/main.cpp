...
struct ChatMessage {
  ...
  bool is_system;
  bool is_self;
  bool is_deleted = false; // Tin nhắn đã bị người gửi xoá (Feature: Delete Message)
  bool is_edited = false;  // Tin nhắn đã được người gửi chỉnh sửa (Feature: Edit Message)
};
...
// --- EDIT MESSAGE STATE (Feature: Edit Message) ---
string editingMessageId = "";      // ID tin nhắn đang được chỉnh sửa (rỗng = không có)
char editMessageBuffer[1024] = ""; // Bộ đệm chứa nội dung khi đang chỉnh sửa

// --- UNREAD MESSAGES STATE (Feature: Unread Messages) ---
int unreadMessageCount = 0;         // Số tin nhắn chưa đọc khi người dùng cuộn lên xem tin cũ
bool requestScrollToBottom = false; // Cờ yêu cầu cuộn xuống cuối khung chat ở khung hình kế tiếp
size_t lastCheckedMsgCount = 0;     // Số tin nhắn đã kiểm tra ở khung hình trước, dùng để phát hiện tin mới
...
void stop_network() {
  ...
  // Reset trạng thái của các tính năng mới khi phiên chat kết thúc
  editingMessageId = "";
        ...
        if (!error) {
          ...
          if (!line.empty()) {
            if (line.rfind("[HANDSHAKE]|", 0) == 0) {
              ...
              // Giao thức: [EDIT]|<msg_id>|<nội_dung_mới>  (Feature: Edit Message)
              stringstream ss(line);
              string tag, target_msg_id, newContent;
              getline(ss, tag, '|');
              getline(ss, target_msg_id, '|');
              getline(ss, newContent); // Phần còn lại của dòng là nội dung mới

              lock_guard<mutex> lock(chat_mutex);
              for (auto &msg : chatHistoryList) {
                // Chỉ áp dụng khi tìm thấy đúng ID và tin nhắn đó thuộc về Peer
                // (không cho phép Peer chỉnh sửa tin nhắn do chính mình gửi)
                if (msg.id == target_msg_id && !msg.is_self) {
                  msg.content = newContent;
                  msg.is_edited = true;
                  break;
                }
              }
              // Nếu không tìm thấy msg_id tương ứng, đơn giản là bỏ qua (không crash)
            } else if (line.rfind("[DELETE]|", 0) == 0) {
              // Giao thức: [DELETE]|<msg_id>  (Feature: Delete Message)
              stringstream ss(line);
              string tag, target_msg_id;
              getline(ss, tag, '|');
              getline(ss, target_msg_id, '|');
              ...
              for (auto &msg : chatHistoryList) {
                // Chỉ áp dụng khi tìm thấy đúng ID và tin nhắn đó thuộc về Peer
                // (không cho phép Peer xoá tin nhắn do chính mình gửi)
                if (msg.id == target_msg_id && !msg.is_self) {
...
void render_chat_bubble(size_t idx, ChatMessage &msg) {
                    ...
                    ImGuiWindowFlags_NoScrollbar);
  {
    ...
    if (editingMessageId == msg.id) {
      // --- Chế độ chỉnh sửa tin nhắn (Feature: Edit Message) ---
      ImGui::PushItemWidth(max(150.0f, bubbleWidth - 30.0f));
      ImGui::InputText(("##editbox_" + msg.id).c_str(), editMessageBuffer,
                       IM_ARRAYSIZE(editMessageBuffer));
      ImGui::PopItemWidth();

      if (ImGui::SmallButton("Save")) {
        string newContent = editMessageBuffer;
        if (!newContent.empty()) {
          msg.content = newContent;
          msg.is_edited = true;
          send_raw_line("[EDIT]|" + msg.id + "|" + newContent);
        }
        editingMessageId = "";
      }
      ImGui::SameLine();
      if (ImGui::SmallButton("Cancel")) {
        editingMessageId = ""; // Huỷ chỉnh sửa, giữ nguyên nội dung cũ
      }
    } else if (msg.is_deleted) {
      // --- Tin nhắn đã bị xoá (Feature: Delete Message) ---
      ImGui::TextColored(ImVec4(0.55f, 0.60f, 0.68f, 1.00f), "%s",
                         msg.content.c_str());
    } else {
      ImGui::TextWrapped("%s", msg.content.c_str());
      if (msg.is_edited) {
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(0.55f, 0.60f, 0.68f, 1.00f), "(edited)");
      }
    }
    ...
    // Ẩn toàn bộ thanh nút này khi tin nhắn đã bị xoá hoặc đang trong chế độ chỉnh sửa
    if (!msg.is_deleted && editingMessageId != msg.id) {
      ...
      // --- Copy / Edit / Delete Buttons (Features: Copy, Edit, Delete Message) ---
      ImGui::Spacing();
      if (ImGui::SmallButton("Copy")) {
        // Sao chép nội dung tin nhắn vào clipboard hệ thống (dùng clipboard của GLFW/Windows)
        ImGui::SetClipboardText(msg.content.c_str());
      }
      if (msg.is_self) {
        // Chỉ chủ tin nhắn mới thấy nút Edit / Delete
        ImGui::SameLine();
        if (ImGui::SmallButton("Edit")) {
          editingMessageId = msg.id;
          strcpy_s(editMessageBuffer, sizeof(editMessageBuffer),
                   msg.content.c_str());
        }
        ImGui::SameLine();
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // --- CONNECTING / CONNECTED / DISCONNECTED ---
      ...
      {
        // Ghi nhận việc người dùng có đang ở cuối khung chat hay không, TRƯỚC KHI
        // các tin nhắn mới của khung hình này được vẽ thêm vào (Feature: Unread Messages)
        bool wasAtBottom =
            ImGui::GetScrollY() >= ImGui::GetScrollMaxY() - 5.0f;

        lock_guard<mutex> lock(chat_mutex);

        // Phát hiện tin nhắn mới được thêm vào kể từ lần kiểm tra trước
        if (chatHistoryList.size() > lastCheckedMsgCount) {
          for (size_t i = lastCheckedMsgCount; i < chatHistoryList.size();
               i++) {
            ChatMessage &newMsg = chatHistoryList[i];
            // Chỉ tính là "tin nhắn mới" đối với tin do Peer gửi tới
            // (không tính tin tự gửi hoặc tin hệ thống)
            if (!newMsg.is_self && !newMsg.is_system) {
              if (wasAtBottom)
                requestScrollToBottom = true; // Đang ở cuối -> tự cuộn xuống
              else
                unreadMessageCount++; // Không ở cuối -> tăng số tin chưa đọc
            }
          }
          lastCheckedMsgCount = chatHistoryList.size();
        }
      ...
      if (ImGui::GetScrollY() >= ImGui::GetScrollMaxY() ||
        ...
        // Đã cuộn tới cuối khung chat -> không còn tin nhắn chưa đọc
        unreadMessageCount = 0;
        requestScrollToBottom = false;
      }
      ImGui::EndChild();

      // --- Thông báo Tin nhắn chưa đọc (Feature: Unread Messages) ---
      if (unreadMessageCount > 0) {
        ...
        if (ImGui::Button(unreadLabel.c_str(), ImVec2(btnWidth, 0))) {
          // Bấm vào thông báo -> yêu cầu cuộn xuống cuối ở khung hình kế tiếp
