...
size_t unreadTrackingIndex;

bool scrollToBottomRequested;
// INPUT BUFFERS
char targetIP[128];
char portBuf[16];
char joinPortBuf[16];
...
// TCP ASIO OBJECTS
asio::io_context* io_context_ptr;
tcp::socket* peer_socket;
tcp::acceptor* peer_acceptor;
...
void start_connecting(string ip, int port) {
    ...
    try {
        ...
        peer_socket->async_connect(endpoint, [ip, port](const asio::error_code& error) { if (!error) { add_system_log("Connected successfully to Host!"); currentState = CONNECTED; peer_ip = ip; peer_tcp_port = port; send_handshake(); async_read_loop(); }
        else {
