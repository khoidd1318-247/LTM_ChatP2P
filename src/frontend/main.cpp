...
void safe_copy(char* destination, size_t destinationSize, const string& source) {
#if defined(_MSC_VER)
...
void render_avatar(const string& name, const string& id, float size = 32.0f) {
  ...
  if (!name.empty() && name != "Anonymous") {
    initial = string(1, static_cast<char>( toupper( static_cast<unsigned char>( name[0]))));
  } else if (!id.empty()) {
  initial = string(1, static_cast<char>( toupper( static_cast<unsigned char>( id[0]))));
}
...
void update_discovery_config() {
  lock_guard<mutex> lock( discoveryConfigMutex );
  discoveryName = (strlen(username) > 0) ? string(username) : "Anonymous";
  discoveryRole = local_role;
  discoveryTcpPort = local_tcp_port;
}
// RECENT PEER STORAGE
void save_recent_peers() {
  ...
  for (const auto& peer : recentPeers) {
    file << sanitize_field(peer.id) << "|" << sanitize_field(peer.name) << "|" << sanitize_field(peer.ip) << "|" << peer.port << "|" << sanitize_field(peer.role) << "|" << sanitize_field(peer.last_connected) << "\n";
  } }
...
if (!peer.id.empty() && !peer.ip.empty()) {
  recentPeers.push_back(peer);
} } }
void remember_peer( const string& id, const string& name, const string& ip, int port, const string& role) {
  if (id.empty() || ip.empty()) return;
  lock_guard<mutex> lock(peer_mutex);
  auto it = find_if( recentPeers.begin(), recentPeers.end(), [&](const RecentPeer& peer) { return peer.id == id || (peer.ip == ip && peer.port == port); } );
  RecentPeer newPeer;
  newPeer.id = id;
  newPeer.name = name.empty() ? "Anonymous" : name;
  newPeer.ip = ip;
  newPeer.port = port;
  newPeer.role = role;
  newPeer.last_connected = get_current_time_str();
  if (it != recentPeers.end()) {
    *it = newPeer;
  } else {
  recentPeers.insert( recentPeers.begin(), newPeer );
}
if (recentPeers.size() > 20) recentPeers.resize(20);
}
// SAVE RECENT PEER WITHOUT DEADLOCK
void remember_peer_and_save( const string& id, const string& name, const string& ip, int port, const string& role) {
  {
    lock_guard<mutex> lock(peer_mutex);
    auto it = find_if( recentPeers.begin(), recentPeers.end(), [&](const RecentPeer& peer) { return peer.id == id || (peer.ip == ip && peer.port == port); } );
    RecentPeer newPeer;
...
string build_discovery_packet() {
  lock_guard<mutex> lock( discoveryConfigMutex );
  string name = sanitize_field(discoveryName);
  string role = sanitize_field(discoveryRole);
  return "[DISCOVER]|" + local_user_id + "|" + name + "|" + role + "|" + to_string(discoveryTcpPort);
}
string build_discovery_reply() {
  lock_guard<mutex> lock( discoveryConfigMutex );
  string name = sanitize_field(discoveryName);
  string role = sanitize_field(discoveryRole);
  return "[DISCOVER_REPLY]|" + local_user_id + "|" + name + "|" + role + "|" + to_string(discoveryTcpPort);
}
// UPDATE DISCOVERED PEER
void update_discovered_peer( const string& id, const string& name, const string& ip, int port, const string& role) {
  if (id.empty() || id == local_user_id || ip.empty()) {
    return;
  }
lock_guard<mutex> lock(peer_mutex);
auto it = find_if( discoveredPeers.begin(), discoveredPeers.end(), [&](const PeerInfo& peer) { return peer.id == id; } );
PeerInfo peer;
peer.id = id;
peer.name = name.empty() ? "Anonymous" : name;
peer.ip = ip;
peer.port = (port >= 1 && port <= 65535) ? port : DEFAULT_CHAT_PORT;
peer.role = role.empty() ? "Idle" : role;
peer.last_seen = get_current_time_str();
if (it != discoveredPeers.end()) {
  *it = peer;
} else {
discoveredPeers.push_back(peer);
}
if (discoveredPeers.size() > 50) discoveredPeers.erase( discoveredPeers.begin() );
}
// FORCE DISCOVERY SCAN
void request_peer_discovery() {
  discoveryScanRequested = true;
  lock_guard<mutex> lock(peer_mutex);
  discoveryStatus = "Searching for peers...";
}
// DISCOVERY SERVICE
void discovery_loop() {
  try {
    ...
    socket.bind( udp::endpoint( udp::v4(), DISCOVERY_PORT ) );
    socket.non_blocking(true);
    udp::endpoint broadcastEndpoint( asio::ip::address_v4::broadcast(), DISCOVERY_PORT );
    discoveryStatus = "Discovery active";
    auto lastBroadcast = chrono::steady_clock::now() - chrono::seconds(5);
    while (discoveryRunning) {
      auto now = chrono::steady_clock::now();
      bool forceScan = discoveryScanRequested.exchange( false );
      if (forceScan || chrono::duration_cast< chrono::milliseconds>( now - lastBroadcast ).count() >= 2000) {
        string packet = build_discovery_packet();
        asio::error_code sendError;
        socket.send_to( asio::buffer(packet), broadcastEndpoint, 0, sendError );
        if (sendError) {
          discoveryError = "UDP discovery send error: " + sendError.message();
        }
      lastBroadcast = now;
    }
  char data[2048];
  ...
  size_t length = socket.receive_from( asio::buffer(data), sender, 0, receiveError );
  if (!receiveError && length > 0) {
    string packet( data, length );
    ...
    int remotePort = DEFAULT_CHAT_PORT;
    try {
      remotePort = stoi(remotePortString);
    } catch (...) {
  }
string remoteIp = sender.address().to_string();
if (remoteId != local_user_id && !remoteId.empty()) {
  update_discovered_peer( remoteId, remoteName, remoteIp, remotePort, remoteRole );
  // If someone is searching,
  // answer their discovery packet.
  if (tag == "[DISCOVER]") {
    string reply = build_discovery_reply();
    asio::error_code replyError;
    socket.send_to( asio::buffer(reply), sender, 0, replyError );
  } } } else if (receiveError != asio::error::would_block && receiveError != asio::error::try_again) {
discoveryError = "UDP discovery receive error: " + receiveError.message();
}
...
discoveryError = string("Discovery service error: ") + e.what();
discoveryStatus = "Discovery unavailable";
} }
void start_discovery_service() {
  if (discoveryRunning) return;
  discoveryRunning = true;
  discoveryThread = thread(discovery_loop);
}
...
void send_handshake() {
  ...
  send_raw_line( "[HANDSHAKE]|" + local_user_id + "|" + display_name + "|" + local_role + "|" + to_string(local_tcp_port) );
}
// TYPING
  ...
  if (!error) {
    ...
    if (!line.empty()) {
      ...
      if (line.rfind( "[HANDSHAKE]|", 0) == 0) {
        ...
        getline( ss, remote_port_string );
        int remotePort = DEFAULT_CHAT_PORT;
        try {
          remotePort = stoi( remote_port_string );
        } catch (...) {
      }
    peer_user_id = remote_id;
    peer_username = remote_name;
    peer_role = remote_role;
    peer_tcp_port = remotePort;
    string connectionIP = peer_ip;
    if (connectionIP.empty() && peer_socket) {
      asio::error_code epError;
      auto endpoint = peer_socket->remote_endpoint( epError );
      if (!epError) {
        connectionIP = endpoint.address() .to_string();
        peer_ip = connectionIP;
      } }
  add_system_log( remote_name + " (" + remote_id + " - " + remote_role + ") joined the chat session" );
  if (!connectionIP.empty()) {
    remember_peer_and_save( remote_id, remote_name, connectionIP, remotePort, remote_role );
  } }
// MESSAGE
...
// REACTION
  ...
  for (auto& msg : chatHistoryList) {
    if (msg.id == target_msg_id) {
      auto it = find_if( msg.reactions.begin(), msg.reactions.end(), [&](const ReactionItem& item) { return item.emoji == emoji && item.user_id == reactor_id; } );
...
// CONNECTION ERROR
  ...
  if (local_role == "Host") {
    add_system_log( "Peer disconnected. Host remains open waiting for a new connection." );
    if (peer_socket) {
      asio::error_code ec;
  ...
  if (peer_acceptor && peer_acceptor->is_open() && peer_socket) {
    ...
    *peer_socket,
    [](const asio::error_code& accept_ec) {
      if (!accept_ec) {
        ...
        if (peer_socket) {
          auto ep = peer_socket->remote_endpoint( endpointError );
          if (!endpointError) {
...
currentState = IDLE;
local_role = "Idle";
local_tcp_port = DEFAULT_CHAT_PORT;
peer_username = "";
peer_user_id = "";
peer_role = "";
peer_ip = "";
...
try {
  ...
  peer_acceptor = new tcp::acceptor(
  *io_context_ptr,
  tcp::endpoint( tcp::v4(), port ) );
  peer_socket = new tcp::socket(
  *io_context_ptr
  );
  currentState = WAITING_FOR_PEER;
  ...
  *peer_socket,
  [](const asio::error_code& error) {
    if (!error) {
      asio::error_code endpointError;
      if (peer_socket) {
        auto endpoint = peer_socket->remote_endpoint( endpointError );
        if (!endpointError) {
          peer_ip = endpoint.address() .to_string();
        } }
...
network_thread = new thread([]() { try { if (io_context_ptr) io_context_ptr->run(); } catch (...) { } });
} catch (exception& e) {
...
try {
  ...
  *io_context_ptr
  ...
  peer_socket->async_connect( endpoint, [ip, port]( const asio::error_code& error) { if (!error) { add_system_log( "Connected successfully to Host!" ); currentState = CONNECTED; peer_ip = ip; peer_tcp_port = port; send_handshake(); async_read_loop(); } else { if (error == asio::error::connection_refused) { statusErrorMessage = "Room unavailable — Host is not ready or Port is incorrect"; } else if ( error == asio::error::timed_out) { statusErrorMessage = "Connection timed out — Please check target IP address"; } else { statusErrorMessage = "Join failed: " + error.message(); }
      add_system_log( "Connection error: " + statusErrorMessage ); currentState = IDLE; local_role = "Idle"; update_discovery_config(); } } );
  network_thread = new thread([]() { try { if (io_context_ptr) io_context_ptr->run(); } catch (...) { } });
} catch (exception& e) {
...
if (ImGui::SmallButton("Save") || editEnterPressed) {
  if (strlen(editMessageBuf) > 0) {
    ...
    msg.is_edited = true;
    send_raw_line( "[EDIT]|" + msg.id + "|" + sanitize_field( msg.content ) );
  }
editingMessageId = "";
}
ImGui::SameLine();
...
if (ImGui::SmallButton("Edit")) {
  ...
  safe_copy( editMessageBuf, sizeof(editMessageBuf), msg.content );
}
ImGui::SameLine();
...
bool peer_matches_search( const PeerInfo& peer, const string& filter) {
  if (filter.empty()) return true;
  string text = peer.name + " " + peer.id + " " + peer.ip + " " + peer.role;
...
// RENDER RECENT PEER
bool recent_peer_matches_search( const RecentPeer& peer, const string& filter) {
  if (filter.empty()) return true;
  string text = peer.name + " " + peer.id + " " + peer.ip + " " + peer.role;
  transform( text.begin(), text.end(), text.begin(), [](unsigned char c) { return static_cast<char>( tolower(c) ); } );
  return text.find(filter) != string::npos;
...
int main() {
  ...
  GLFWwindow* window = glfwCreateWindow( 900, 860, "P2P Chat - Peer Discovery", NULL, NULL );
  if (!window) {
    glfwTerminate();
    return -1;
  }
...
if (fs::exists( "C:\\Windows\\Fonts\\seguiemj.ttf")) {
  ...
  static const ImWchar emoji_ranges[] = {
    0x2000, 0x3300, 0x2600, 0x27FF, 0xE000, 0xFFFD, 0 };
...
if (currentState == IDLE) {
  ...
  ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "Peer Discovery & Direct Peer-to-Peer Messaging" );
  ImGui::Spacing();
  // ERROR
...
// USER PROFILE
ImGui::BeginChild( "ProfileSection", ImVec2(0, 105), true );
ImGui::TextColored( ImVec4( 0.45f, 0.75f, 1.0f, 1.0f ), "Your Profile" );
ImGui::Spacing();
...
ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "Your unique peer identifier" );
update_discovery_config();
ImGui::EndChild();
ImGui::Spacing();
// TWO COLUMN LAYOUT
...
float rightWidth = availableWidth - leftWidth - 12.0f;
// LEFT: CONTACT / DISCOVERY
ImGui::BeginChild( "ContactManager", ImVec2(leftWidth, 0), true );
ImGui::TextColored( ImVec4( 0.45f, 0.75f, 1.00f, 1.00f ), "Contacts / Peer List" );
ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "Find friends on the same LAN" );
ImGui::Spacing();
// PEER SEARCH - SEPARATE FROM CHAT SEARCH
ImGui::InputTextWithHint( "##peerSearch", "Search contacts / peers...", peerSearchBuf, IM_ARRAYSIZE(peerSearchBuf) );
ImGui::SameLine();
if (ImGui::Button( "Find Friends", ImVec2(115, 0))) {
  request_peer_discovery();
}
ImGui::Spacing();
ImGui::TextColored( ImVec4( 0.40f, 0.85f, 0.50f, 1.00f ), "Discovery: %s", discoveryStatus.c_str() );
if (!discoveryError.empty()) {
  ImGui::TextColored( ImVec4( 1.0f, 0.45f, 0.45f, 1.0f ), "%s", discoveryError.c_str() );
}
ImGui::Spacing();
ImGui::Separator();
ImGui::Text( "Discovered Peers" );
ImGui::Spacing();
string peerFilter = peerSearchBuf;
transform( peerFilter.begin(), peerFilter.end(), peerFilter.begin(), [](unsigned char c) { return static_cast<char>( tolower(c) ); } );
ImGui::BeginChild( "DiscoveredPeerList", ImVec2(0, 260), true );
{
  lock_guard<mutex> lock( peer_mutex );
  bool foundAny = false;
  for (const auto& peer : discoveredPeers) {
    if (!peer_matches_search( peer, peerFilter)) {
      continue;
    }
  foundAny = true;
  ImGui::PushID( ( "discover_" + peer.id ).c_str() );
  render_avatar( peer.name, peer.id, 34.0f );
  ImGui::SameLine();
  ImGui::BeginGroup();
  ImGui::Text( "%s %s", peer.name.c_str(), peer.id.c_str() );
  ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "%s:%d • %s • Seen %s", peer.ip.c_str(), peer.port, peer.role.c_str(), peer.last_seen.c_str() );
  ImGui::EndGroup();
  ImGui::SameLine( ImGui::GetContentRegionAvail().x - 105 );
  bool canConnect = peer.role == "Host";
  if (!canConnect) ImGui::BeginDisabled();
  if (ImGui::Button( "Connect", ImVec2(90, 30))) {
    safe_copy( targetIP, sizeof(targetIP), peer.ip );
    safe_copy( joinPortBuf, sizeof(joinPortBuf), to_string( peer.port ) );
    start_connecting( peer.ip, peer.port );
  }
...
if (!foundAny) {
  ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "No peers found." );
  ImGui::TextWrapped( "Click \"Find Friends\" to search for P2P Chat users on your local network." );
} }
ImGui::EndChild();
ImGui::Spacing();
// RECENT PEERS
ImGui::Text( "Recent Peers" );
ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "Reconnect with one click" );
ImGui::BeginChild( "RecentPeerList", ImVec2(0, 190), true );
{
  lock_guard<mutex> lock( peer_mutex );
  bool foundRecent = false;
  for (size_t i = 0; i < recentPeers.size(); ++i) {
    const auto& peer = recentPeers[i];
    if (!recent_peer_matches_search( peer, peerFilter)) {
      continue;
    }
  foundRecent = true;
  ImGui::PushID( ( "recent_" + to_string(i) ).c_str() );
  render_avatar( peer.name, peer.id, 30.0f );
  ImGui::SameLine();
  ImGui::BeginGroup();
  ImGui::Text( "%s %s", peer.name.c_str(), peer.id.c_str() );
  ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "%s:%d • Last: %s", peer.ip.c_str(), peer.port, peer.last_connected.c_str() );
  ImGui::EndGroup();
  ImGui::SameLine( ImGui::GetContentRegionAvail().x - 105 );
  if (ImGui::Button( "Reconnect", ImVec2(90, 30))) {
    safe_copy( targetIP, sizeof(targetIP), peer.ip );
    safe_copy( joinPortBuf, sizeof(joinPortBuf), to_string( peer.port ) );
    start_connecting( peer.ip, peer.port );
  }
...
if (!foundRecent) {
  ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "No recent peers yet." );
} }
ImGui::EndChild();
ImGui::EndChild();
ImGui::SameLine();
// RIGHT: CONNECTION METHODS
ImGui::BeginChild( "ConnectionManager", ImVec2(rightWidth, 0), true );
ImGui::TextColored( ImVec4( 0.45f, 0.75f, 1.00f, 1.00f ), "Connection" );
ImGui::Spacing();
// HOST
ImGui::Text( "Host Room" );
ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "Create a room for another peer" );
ImGui::Spacing();
int parsedPort = DEFAULT_CHAT_PORT;
...
if (!validPort) {
  ImGui::TextColored( ImVec4( 1.0f, 0.4f, 0.4f, 1.0f ), "Port must be 1-65535" );
}
ImGui::BeginDisabled( !validPort );
if (ImGui::Button( "Host Room", ImVec2(-1, 40))) {
...
ImGui::Spacing();
// MANUAL JOIN
ImGui::Text( "Manual Join" );
ImGui::TextColored( ImVec4( 0.55f, 0.60f, 0.68f, 1.00f ), "Fallback if discovery is unavailable" );
ImGui::Spacing();
...
int parsedJoinPort = DEFAULT_CHAT_PORT;
bool validJoinPort = isValidPort( joinPortBuf, parsedJoinPort );
if (!validIP) {
  ImGui::TextColored( ImVec4( 1.0f, 0.4f, 0.4f, 1.0f ), "Invalid IP" );
}
if (!validJoinPort) {
  ImGui::TextColored( ImVec4( 1.0f, 0.4f, 0.4f, 1.0f ), "Invalid Port" );
}
ImGui::BeginDisabled( !validIP || !validJoinPort );
if (ImGui::Button( "Join Room", ImVec2(-1, 40))) {
...
ImGui::TextColored( ImVec4( 0.45f, 0.75f, 1.00f, 1.00f ), "Peer Discovery" );
ImGui::TextWrapped( "UDP discovery port: %d", DISCOVERY_PORT );
ImGui::TextWrapped( "Discovery works automatically on the same LAN/Wi-Fi." );
ImGui::Spacing();
if (ImGui::Button( "Scan Again", ImVec2(-1, 36))) {
  request_peer_discovery();
}
ImGui::EndChild();
}
// CONNECTED / WAITING
  ...
  ImGui::TextColored( ImVec4( 1.0f, 0.8f, 0.3f, 1.0f ), "Waiting for Peer..." );
} else {
...
if (ImGui::Button( showInfoPanel ? "Hide Info" : "Room Info", ImVec2(85, 30))) {
...
if (showInfoPanel) {
  ...
  ImGui::Text( "Protocol: TCP P2P | Discovery: UDP %d", DISCOVERY_PORT );
  if (local_role == "Peer") {
    ImGui::Text( "Host IP: %s | Peer ID: %s", peer_ip.c_str(), peer_user_id.empty() ? "Connecting..." : peer_user_id.c_str() );
  } else {
  ImGui::Text( "Hosting Port: %d | Connected Peer: %s", local_tcp_port, peer_username.empty() ? "None" : ( peer_username + " " + peer_user_id ).c_str() );
}
ImGui::EndChild();
ImGui::PopStyleColor();
ImGui::Spacing();
...
//
ImGui::TextColored( ImVec4( 0.45f, 0.75f, 1.00f, 1.00f ), "Chat History Search" );
ImGui::SameLine();
...
// CHAT REGION
float bottomPadding = currentState == CONNECTED ? 175.0f : 60.0f;
...
for ( size_t idx = 0; idx < chatHistoryList.size(); idx++) {
  ...
  if (!filterStr.empty()) {
    ...
    transform( contentLower.begin(), contentLower.end(), contentLower.begin(), [](unsigned char c) { return static_cast<char>( tolower(c) ); } );
    if (contentLower.find( filterStr ) == string::npos) {
      continue;
    } }
if (msg.is_system) {
  ImGui::Spacing();
  float textWidth = ImGui::CalcTextSize( msg.content.c_str() ).x;
  ImGui::SetCursorPosX( max( 0.0f, ( ImGui::GetWindowWidth() - textWidth ) * 0.5f ) );
...
const char* quickReplies[] = {
  "Hello!", "I'm on my way!", "Sounds good!", "Call you later", "Got it!", "Thanks!" };
for (int i = 0; i < 6; i++) {
  if (i > 0) ImGui::SameLine();
  if (ImGui::SmallButton( quickReplies[i])) {
    safe_copy( messageBuf, sizeof(messageBuf), quickReplies[i] );
  } }
// QUICK EMOTES
...
if ( ImGui::Button( "Send", ImVec2(60, 0) ) || isInputChanged) {
  if (strlen(messageBuf) > 0) {
    ...
    string safe_name = sanitize_field( display_name );
    string safe_r_name = sanitize_field( replyTargetName );
    string safe_r_text = sanitize_field( replyTargetText );
    string content = string(messageBuf);
    string packet = "[MSG]|" + new_msg_id + "|" + local_user_id + "|" + safe_name + "|" + local_role + "|" + ts + "|" + safe_r_name + "|" + safe_r_text + "|" + content;
    send_raw_line( packet );
    add_chat_log( new_msg_id, display_name, local_user_id, local_role, ts, replyTargetName, replyTargetText, content, true );
    messageBuf[0] = '\0';
    replyTargetId = "";
    replyTargetName = "";
    replyTargetText = "";
